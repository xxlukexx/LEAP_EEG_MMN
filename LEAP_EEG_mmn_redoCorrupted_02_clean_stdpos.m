...
% try
    ...
    % output path
    path_clean = '/Volumes/projects/LEAP/EEG/mmn/_redo_corrupted/02_clean_stdpos_100hz';
    tryToMakePath(path_clean)

    % input path
    path_preproc = '/Volumes/projects/LEAP/EEG/mmn/_redo_corrupted/01_preproc_stdpos_100Hz';
...
%     options.alpha = false;
    ...
    file_diary = fullfile(path_preproc, sprintf('log_%s.txt', datestr(now, 30)));
    diary(file_diary)
    ...
    for f = 1:numSched
        if ~strcmp(d(f).name(1), '_')
            ...
            ops = struct;
            fut(futCounter) =...
                parfeval(@LEAP_EEG_mmn_02_doClean, 3, ops, path_in,...
                ids_all{f}, path_clean, [], [], []);
            
%             [dta, fo, opz] = LEAP_EEG_mmn_02_doClean(ops, path_in,...
%                 ids_all{f}, path_clean, [], [], []);
...
%         {'Met_Cutoff_TPC20', 'Met_Cutoff_Interp10', 'Met_Cutoff_Excl10', 'Met_All_Cutoffs'})];
    ...
    res.Met_All_Cutoffs = true(size(res, 1), 1);
    ...
    save([path_clean, filesep, '_results.mat'], 'res', 'summaries');
